function [laser_distances, imageOut] = laser_scan_model(x, y, min_range, max_range, ...
    number_of_rays, min_angle, max_angle, map_image, map_image_rgb, occupied_threshold, distance_step, ...
    lidar_rgb, gaussian_power, generate_image, pixelsToAdd)
...
y_pixels = zeros(1, number_of_rays);
    ...
    for laserScanIter=1:number_of_rays
        ...
        angle = min_angle + laserScanIter * (max_angle - min_angle) / number_of_rays;
        ...
        while distance <= max_range
            ...
            if (((xMap || yMap) < 1) || xMap > map_width || yMap > map_height)
            %We're off the map! Probably need to just quit here and record max 
            %range            
                distance = max_range;
            break;
            ...
            if ( double(map_image(ceil(yMap), ceil(xMap))) < occupied_threshold )
            %We've identified an occupied cell, record this distance and move
            % on
                break;
        ...
        for i = 1 :  map_width
                for j = 1 : map_height
                    if max(i == x_pixels & j == y_pixels) > 0.5
                        imageOut(j,i,:) = cat(3, lidar_rgb(1), lidar_rgb(2),...
                            lidar_rgb(3)); 
                    else 
